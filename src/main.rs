...
fn get_mp_size(path: &Path) -> Result<i64> {
    ...
    let xmp = file.xmp().ok_or_else(|| anyhow!("File has no XMP data"))?;

    let containers_len = xmp.array_len(NS_CONTAINER, "Directory");

    for i in 1..=containers_len {
        let array_item_path =
            XmpMeta::compose_array_item_path(NS_CONTAINER, "Directory", i.try_into().unwrap())?;
        let item_path = XmpMeta::compose_struct_field_path(
            NS_CONTAINER,
            &array_item_path,
            NS_CONTAINER,
            "Item",
        ...
        let mime_path =
            XmpMeta::compose_struct_field_path(NS_CONTAINER, &item_path, NS_ITEM, "Mime")?;
        let length_path =
            XmpMeta::compose_struct_field_path(NS_CONTAINER, &item_path, NS_ITEM, "Length")?;
        let semantic_path =
            XmpMeta::compose_struct_field_path(NS_CONTAINER, &item_path, NS_ITEM, "Semantic")?;

        let semantic = xmp
            .property(NS_CONTAINER, &semantic_path)
        ...
        let mime = xmp
            .property(NS_CONTAINER, &mime_path)
            .ok_or_else(|| anyhow!("Directory item {i} has no MIME type"))?;
        if mime.value != "video/mp4" {
            return Err(anyhow!(
                "Unexpected MIME type for motion photo: {:#?}",
                mime.value
            ));
        }

        let length = xmp
            .property_i64(NS_CONTAINER, &length_path)
            .ok_or_else(|| anyhow!("Directory item {i} has no length field"))?;

        return Ok(length.value);
    }

    bail!("Motion photo directory item not found")
}

fn copy_from_end<R: Read + Seek, W: Write>(
    input: &mut R,
    output: &mut W,
    mut length: i64,
) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    let length = get_mp_size(&opts.input)
        .with_context(|| format!("Failed to get embedded video size from: {:?}", opts.input))?;

    let mut input = File::open(&opts.input)
        .with_context(|| format!("Failed to open for reading: {:?}", opts.input))?;
